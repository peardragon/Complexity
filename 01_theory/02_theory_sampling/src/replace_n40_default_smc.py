...
import build_default_smc_final as final_tables


PROJECT_ROOT = Path(__file__).resolve().parents[3]
SUMMARY_ROOT = PROJECT_ROOT / "01_theory" / "02_theory_sampling" / "summarized_outputs"
SHELL_SUMMARY_ROOT = SUMMARY_ROOT / "shell_pool"
RAW_SHELL_ROOT = PROJECT_ROOT / "01_theory" / "02_theory_sampling" / "raw_outputs" / "shell_pool"
SOURCE_DATA_ROOT = (
    PROJECT_ROOT.parent
    / "windows_project"
    / "01_theory"
    ...
    / "shell_pool"
    / "sample_payloads"
    / "base_10x10_N40_80_160_320_p2048_r0p05"
    / "N_0040"
)

DEFAULT_METHOD = "exact_shell_l2_vmf_adaptive_ce_smc"
RUN_ID = "two_pool_perceptron_alpha0p1_N40_default_smc_tempered_path_replacement"
...
def load_dataset_and_reference(dataset_id: int, ref_id: int) -> tuple[np.ndarray, np.ndarray]:
    dataset_dir = SOURCE_DATA_ROOT / f"dataset_{int(dataset_id):03d}"
    if not dataset_dir.exists():
        raise FileNotFoundError(dataset_dir)
    a_matrix = np.load(dataset_dir / "dataset.npz")["A"].astype(np.float64)
    theta_ref = np.load(dataset_dir / "references.npz")["references"][int(ref_id)].astype(np.float64)
...
def compute_unit(
    ...
    *,
    ...
    n_value = int(row["N"])
    if n_value != 40:
        raise ValueError(f"expected N=40 row, got N={n_value}")
    dataset_id = int(row["dataset_id"])
    ref_id = int(row["ref_id"])
    radius = float(row["radius"])
    n_particles = int(row["n_particles"])
    a_matrix, theta_ref = load_dataset_and_reference(dataset_id, ref_id)
    dim = int(theta_ref.size)
    if dim != n_value:
    ...
    row_update = {
        ...
        "replacement_reason": "n40_default_smc_tempered_path",
        "replacement_source_path": "",
        "replacement_from_sampler_method": row.get("sampler_method", ""),
        "replacement_from_n_particles": n_particles,
...
def rebuild_tables(units: pd.DataFrame, *, original_unit_count: int, replacement_unit_count: int) -> dict[str, Any]:
    ...
    validation = final_tables.validation_summary(
        ...
        qc,
        base={
            "original_unit_count": int(original_unit_count),
            "replacement_unit_count": int(replacement_unit_count),
            "target_unit_count": int(replacement_unit_count),
            "target_nondefault_method_count": int((units["sampler_method"] != DEFAULT_METHOD).sum()),
            "target_over_particle_cap_count": int((units["n_particles"] > 32768).sum()),
        },
        particle_cap=32768,
        split_threshold=0.006,
        smc_cess_threshold=0.8,
    )
    SHELL_SUMMARY_ROOT.mkdir(parents=True, exist_ok=True)
    units.to_csv(SHELL_SUMMARY_ROOT / "sample_unit_summary.csv", index=False)
    phi.to_csv(SHELL_SUMMARY_ROOT / "sampling_phi_by_N_alpha0p1.csv", index=False)
    phi.to_csv(SUMMARY_ROOT / "fig01_sampling_phi_by_distance.csv", index=False)
    qc.to_csv(SHELL_SUMMARY_ROOT / "sampling_logz_stability_by_N_radius.csv", index=False)
    (SHELL_SUMMARY_ROOT / "default_smc_final_validation.json").write_text(
        json.dumps(validation, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    final_tables.write_report(SHELL_SUMMARY_ROOT / "run_report.md", validation)
    return validation


def main() -> None:
    parser = argparse.ArgumentParser(description="Replace all N=40 theory shell units with default CE-tempered SMC.")
    parser.add_argument("--summary", type=Path, default=SHELL_SUMMARY_ROOT / "sample_unit_summary.csv")
    parser.add_argument("--limit", type=int, default=0, help="Debug limit; 0 means all target rows.")
    parser.add_argument("--start-index", type=int, default=0, help="Debug offset into target rows.")
    ...
    parser.add_argument("--write-summary", action="store_true", help="Write summary tables after processing.")
    ...
    if args.only_index:
        target_indices = [int(index) for index in args.only_index]
    else:
        target_indices = list(units.index[units["N"].astype(int) == 40])
    if int(args.start_index) > 0 and not args.only_index:
        target_indices = target_indices[int(args.start_index) :]
    if int(args.limit) > 0 and not args.only_index:
        target_indices = target_indices[: int(args.limit)]
    print(f"target N=40 rows: {len(target_indices)}")
    if args.dry_run:
        for index in target_indices[:5]:
            row = units.loc[index]
            a_matrix, theta_ref = load_dataset_and_reference(int(row["dataset_id"]), int(row["ref_id"]))
            ce_ref, err_ref = stable_softplus_neg_margin(theta_ref[None, :], a_matrix)
            print(
                json.dumps(
                    {
    ...
    for processed, index in enumerate(target_indices, start=1):
        ...
        if processed == 1 or processed % 100 == 0 or processed == len(target_indices):
            elapsed = sum(float(item["elapsed_s"]) for item in diagnostics)
            print(
                f"processed {processed}/{len(target_indices)} "
    ...
    if not args.write_summary:
        print("payloads written; summary tables not updated because --write-summary was not supplied")
        return

    validation = rebuild_tables(
        units,
        original_unit_count=original_unit_count,
        replacement_unit_count=len(target_indices),
    )
    replacement_csv = SHELL_SUMMARY_ROOT / "n40_default_smc_replacement_units.csv"
    units.loc[units["N"].astype(int) == 40].to_csv(replacement_csv, index=False)
    needed_csv = SHELL_SUMMARY_ROOT / "n40_default_smc_replacement_needed.csv"
    units.loc[units["sampler_method"] != DEFAULT_METHOD].to_csv(needed_csv, index=False)
    audit = [
        "# N=40 default-SMC replacement audit",
        "",
        "All active N=40 theory shell units were regenerated with the default CE-tempered SMC path.",
        "",
        f"- Processed N=40 units in latest run: `{len(target_indices)}`.",
        f"- Current active N=40 default-SMC rows: `{int(((units['N'].astype(int) == 40) & (units['sampler_method'] == DEFAULT_METHOD)).sum())}`.",
        f"- Replacement method: `{DEFAULT_METHOD}`.",
        f"- Source run id: `{RUN_ID}`.",
        f"- Non-default method count after rebuild: `{validation['nondefault_method_count']}`.",
        f"- Fallback unit count after rebuild: `{validation['fallback_unit_count']}`.",
        f"- Full default-SMC pass: `{validation['full_default_smc_pass']}`.",
        f"- Replacement rows CSV: `{replacement_csv.relative_to(PROJECT_ROOT)}`.",
        f"- Remaining-needed manifest rows: `{len(units.loc[units['sampler_method'] != DEFAULT_METHOD])}`.",
    ]
    (SHELL_SUMMARY_ROOT / "n40_default_smc_replacement_audit.md").write_text(
        "\n".join(audit) + "\n",
        encoding="utf-8",
    )
