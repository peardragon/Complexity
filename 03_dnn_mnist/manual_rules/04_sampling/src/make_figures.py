...
try:
    from .make_summarized_outputs import DEFAULT_FIGURE_INPUT_ROOT, clear_outputs, rule_from_name, rule_summary_files
except ImportError:
    from make_summarized_outputs import DEFAULT_FIGURE_INPUT_ROOT, clear_outputs, rule_from_name, rule_summary_files
...
def render_figures(
    ...
    *,
    ...
    files = rule_summary_files(input_root)
    if not files:
        raise FileNotFoundError(f"no rule_*.csv files found under {input_root}")
    output_root.mkdir(parents=True, exist_ok=True)
    clear_outputs(output_root, "rule_*.png")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build manual-rule sampling figures from figure-input CSVs.")
