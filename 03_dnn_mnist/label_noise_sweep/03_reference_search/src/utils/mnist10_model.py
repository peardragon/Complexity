...
def init_parts(rng: np.random.Generator, *, scale_multiplier: float = 1.0, arch: MNIST10Arch = ARCH) -> tuple[np.ndarray, ...]:
...
def init_theta(seed: int, *, scale_multiplier: float = 1.0, arch: MNIST10Arch = ARCH) -> np.ndarray:
    return flatten_parts(init_parts(np.random.default_rng(int(seed)), scale_multiplier=scale_multiplier, arch=arch))


def param_slices(arch: MNIST10Arch = ARCH) -> dict[str, tuple[int, int, tuple[int, ...]]]:
    h = int(arch.hidden_width)
    d = int(arch.input_dim)
    idx = 0
...
def margin_stats_np(theta: np.ndarray, x: np.ndarray, y: np.ndarray, arch: MNIST10Arch = ARCH) -> dict[str, float]:
