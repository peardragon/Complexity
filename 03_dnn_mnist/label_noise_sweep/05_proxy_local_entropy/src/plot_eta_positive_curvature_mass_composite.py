...
OUT_NAME = "eta_positive_curvature_mass_dense30ref_r1p0_n1024_cpu60_gpu0_eta0p02_0p05_0p15_0p25"
DEFAULT_RUN = STAGE_ROOT / "summarized_outputs" / RUN_NAME
DEFAULT_OUT_DIR = STAGE_ROOT / "figures" / OUT_NAME
DEFAULT_SUMMARY_DIR = STAGE_ROOT / "summarized_outputs" / OUT_NAME
...
def main() -> int:
    ...
    ref_metrics.to_csv(summary_dir / "eta_positive_curvature_mass_by_ref.csv", index=False)
    curvature.to_csv(summary_dir / "eta_curvature_by_eta_ref_radius.csv", index=False)
    eta_summary.to_csv(summary_dir / "eta_positive_curvature_mass_by_eta.csv", index=False)
    curve_summary.to_csv(summary_dir / "eta_derivative_curvature_curve_by_eta_radius.csv", index=False)
