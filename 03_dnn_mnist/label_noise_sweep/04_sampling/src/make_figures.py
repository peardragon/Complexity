...
try:
    from .make_summarized_outputs import DEFAULT_FIGURE_INPUT_ROOT, clear_outputs, eta_from_name, eta_summary_files
except ImportError:
    from make_summarized_outputs import DEFAULT_FIGURE_INPUT_ROOT, clear_outputs, eta_from_name, eta_summary_files
...
def render_figures(
    ...
    *,
    ...
    files = eta_summary_files(input_root)
    if not files:
        raise FileNotFoundError(f"no eta_*.csv files found under {input_root}")
    output_root.mkdir(parents=True, exist_ok=True)
    clear_outputs(output_root, "noise_eta_*.png")
    clear_outputs(output_root, "eta_*.png")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build label-noise sampling figures from figure-input CSVs.")
