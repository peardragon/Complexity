...
INNER_ROOT = PROJECT_ROOT / "Complexity"
REPO_ROOT = PROJECT_ROOT / "windows_project"
...
def current_radius_token(radius: float) -> str:
    return f"r_{float(radius):.2f}".replace(".", "p")


def noise_eta_token(rule: str) -> str:
    if str(rule).startswith("noise_eta_"):
        return str(rule)
    if str(rule).startswith("eta_"):
        return f"noise_{rule}"
    return str(rule)


def output_ref_id(row: dict[str, Any]) -> int:
    if "output_ref_id" in row:
        return int(row["output_ref_id"])
    ref_value = int(row["ref_id"])
    return ref_value + 1 if ref_value < 100 else ref_value


def resolve_payload_path(path_text: str | Path) -> Path:
    path = Path(path_text)
    if path.is_absolute():
        return path
    for root in (PROJECT_ROOT, INNER_ROOT, REPO_ROOT):
        candidate = root / path
        if candidate.exists():
            return candidate
...
def unit_summary_path(run_root: Path, row: dict[str, Any], radius: float) -> Path:
    rule = str(row["rule"])
    if rule.startswith("eta_") or rule.startswith("noise_eta_"):
        return (
            run_root
            / noise_eta_token(rule)
            / f"ref_{output_ref_id(row):03d}"
            / current_radius_token(radius)
            / "unit_summary.json"
        )
    rule_dir = str(row.get("rule_id", ""))
    if rule_dir.startswith("rule_"):
        return (
            run_root
            / rule_dir
            / f"ref_{output_ref_id(row):03d}"
...
def sample_unit(row: dict[str, Any], radius: float, cfg: dict[str, Any], run_root: Path, *, force: bool) -> dict[str, Any]:
    ...
    if policy is None:
        n_samples = int(cfg["sampling"]["samples_per_ref_radius"])
        ds = pipe.load_dataset(resolve_payload_path(row["dataset_path"]))
        theta_ref = np.load(resolve_payload_path(row["theta_path"])).astype(np.float64).reshape(-1)
