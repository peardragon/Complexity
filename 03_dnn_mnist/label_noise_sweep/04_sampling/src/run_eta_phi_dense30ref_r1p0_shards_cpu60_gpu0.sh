...
date -Is > "${RUN_ROOT}/run_metadata/MANAGER_STARTED_AT.txt"
: > "${RUN_ROOT}/run_metadata/RUNNING_PIDS.txt"
...
for shard in $(seq 0 $((SHARDS - 1))); do
  ...
  echo "$! shard${shard}_of_${SHARDS}" >> "${RUN_ROOT}/run_metadata/RUNNING_PIDS.txt"
done

status=0
while read -r pid _; do
  wait "${pid}" || status=1
done < "${RUN_ROOT}/run_metadata/RUNNING_PIDS.txt"
...
date -Is > "${RUN_ROOT}/run_metadata/MANAGER_FINISHED_AT.txt"
exit "${status}"
