...
def configure_sampling(
    ...
    cfg["sampling"]["note"] = "Eta-specific exact references from 06_eta_flip_phase_transition reference-search smoke."
...
def sample_tasks(
    ...
    write_csv(
        run_root / "05_pool2_pm_sais_sampling" / f"tasks_shard{shard_index}_of_{shard_count}.csv",
        pd.DataFrame(
            [
                {
                    "task_index": idx,
    ...
    for idx, (row, radius) in enumerate(tasks, start=1):
        ...
        rows.append(payload)
        write_csv(
            run_root / "05_pool2_pm_sais_sampling" / f"sampling_log_latest_shard{shard_index}_of_{shard_count}.csv",
            pd.DataFrame(rows),
        )
    out = pd.DataFrame(rows)
    out["run_elapsed_s"] = float(time.time() - started)
    write_csv(run_root / "05_pool2_pm_sais_sampling" / f"shard{shard_index}_unit_summary.csv", out)
    return out
...
def summarize(run_root: Path, cfg: dict[str, Any], etas: list[float], radii: list[float], ref_count: int) -> dict[str, Any]:
    ...
    write_csv(run_root / "05_pool2_pm_sais_sampling" / "shell_summary_by_unit_with_phi_derivatives.csv", joined)
...
def main() -> int:
    ...
    parser.add_argument("--reference-run-root", type=Path, default=STAGE_ROOT / "raw_outputs" / "eta_reference_search_4eta_3ref_cpu35_gpu0")
    ...
    write_csv(run_root / "04_reference_pool" / "reference_index.csv", refs)
    if not args.aggregate_only:
        ...
        if args.no_final_aggregate:
            ...
            write_json(
                run_root / "05_pool2_pm_sais_sampling" / f"shard{args.shard_index}_status.json",
                status,
            )
