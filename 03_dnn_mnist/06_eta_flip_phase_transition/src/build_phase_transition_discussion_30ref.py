...
def eta_nmstv_map(graph_run: Path, etas: list[float]) -> tuple[dict[float, float], dict[float, str]]:
...
def load_units(
    ...
    required = {
        ...
        "eta_graph_nmstv_table": graph_run / "summary_by_eta_k.csv",
    }
    missing = [str(path) for key, path in required.items() if key != "eta_graph_nmstv_table" and not path.exists()]
    if missing:
        raise FileNotFoundError("Missing required input CSV(s): " + ", ".join(missing))

    nmstv, nmstv_methods = eta_nmstv_map(graph_run, etas)
...
def main() -> int:
    ...
    table_paths = [
        out_dir / "curve_summary_by_case_radius.csv",
        out_dir / "reflevel_derivative_curvature_by_case_radius.csv",
        out_dir / "A_kappa_by_reference.csv",
        out_dir / "case_summary_A_kappa.csv",
        out_dir / "gap_to_even_odd_by_radius.csv",
        out_dir / "gap_metrics_to_even_odd.csv",
    ]
    ...
    input_paths_json = {
        ...
        "graph_run_root": str(args.graph_run_root.resolve()),
    ...
    config = {
        ...
        "outputs": [str(path.resolve()) for path in [*table_paths, *figure_paths, out_dir / "REPORT.md"]],
    }
    write_json(out_dir / "run_config_resolved.json", config)
    write_json(
        out_dir / "provenance_paths.json",
        {
            "input_paths": input_paths_json,
            "input_sha256": input_hashes,
    ...
    build_report(
        ...
        case_summary,
        gap_metrics,
        [*table_paths, *figure_paths, out_dir / "run_config_resolved.json", out_dir / "provenance_paths.json", out_dir / "REPORT.md"],
    )
