...
ETA_LIST="0.02,0.05,0.15,0.25"
REF_COUNT=30
OLD_REF_ROOT="$STAGE/raw_outputs/eta_reference_search_advanced_4eta_90ref_cpu35_gpu0"
LOW_REF_ROOT="$STAGE/raw_outputs/eta_reference_search_gapfill_0p05_0p15_30ref_cpu60_gpu0"
EXTRA_REF_RUN_NAME="eta_reference_search_gapfill_0p02_30ref_cpu60_gpu0"
EXTRA_REF_ROOT="$STAGE/raw_outputs/$EXTRA_REF_RUN_NAME"
REF_ROOT="$STAGE/raw_outputs/eta_reference_search_gapfill_0p02_0p05_0p15_0p25_30ref_cpu60_gpu0"
...
mkdir -p "$LOG_DIR"
date -Is > "$RUN_ROOT/ADD_ETA0P02_STARTED_AT.txt"
: > "$RUN_ROOT/RUNNING_PIDS_ETA0P02.txt"

cat > "$RUN_ROOT/ADD_ETA0P02_RUN_PLAN.txt" <<EOF
run_name=$RUN_NAME
run_root=$RUN_ROOT
reference_run_root=$REF_ROOT
eta_list=$ETA_LIST
scope=add eta 0.02 to existing eta 0.05,0.15,0.25 run; 30 references per eta, n=1024
...
estimate_note=This reuses cached 0.05/0.15/0.25 units and computes only missing eta 0.02 units; expected new expensive units: 3000.
EOF
...
prepare_reference_pool() {
  if needs_reference_pool; then
    echo "[eta0p02-add] reference pool already ready: $REF_ROOT"
    return 0
  fi

  echo "[eta0p02-add] preparing eta=0.02 reference pool"
  if ! python - "$EXTRA_REF_ROOT" <<'PY'
import sys
from pathlib import Path
import pandas as pd
...
if int((refs["eta"].sub(0.02).abs() < 1e-9).sum()) < 30:
    raise SystemExit(1)
raise SystemExit(0)
PY
  then
    ...
    ) > "$LOG_DIR/reference_search_gapfill_0p02.log" 2>&1
  fi

  python - "$REF_ROOT" "$EXTRA_REF_ROOT" "$LOW_REF_ROOT" "$OLD_REF_ROOT" "$ETA_LIST" "$REF_COUNT" <<'PY'
import json
import sys
from pathlib import Path
...
extra_root = Path(sys.argv[2])
low_root = Path(sys.argv[3])
old_root = Path(sys.argv[4])
etas = [float(part) for part in sys.argv[5].split(",") if part.strip()]
ref_count = int(sys.argv[6])
...
extra_refs = load_refs(extra_root)
low_refs = load_refs(low_root)
old_refs = load_refs(old_root)
parts = []
available = {}
for eta in etas:
    if abs(eta - 0.02) < 1e-9:
        source = extra_refs
    elif abs(eta - 0.25) < 1e-9:
        source = old_refs
    else:
        source = low_refs
...
status = {
    ...
    "sources": {
        "eta_0p02": str(extra_root),
        "eta_0p05_0p15": str(low_root),
        "eta_0p25": str(old_root),
    },
    "note": "Merged very-small eta pool: new eta 0.02 exact references plus existing eta 0.05/0.15/0.25 references.",
}
...
for shard in $(seq 0 $((SHARDS - 1))); do
  ...
  echo "$! shard${shard}_of_${SHARDS} cpus_${cpu_list}" >> "$RUN_ROOT/RUNNING_PIDS_ETA0P02.txt"
done

status=0
while read -r pid _; do
  wait "$pid" || status=1
done < "$RUN_ROOT/RUNNING_PIDS_ETA0P02.txt"
...
python "$STAGE/src/plot_eta_phi_energy_only.py" \
  --run-root "$RUN_ROOT" \
  --out-dir "$STAGE/figures/eta_phi_energy_dense30ref_r1p0_n1024_cpu60_gpu0_eta0p02_0p05_0p15_0p25" \
  --d-min 0.01 \
  --d-max 1.0 > "$LOG_DIR/plot_eta_phi_energy.log" 2>&1 || status=1

python "$STAGE/src/plot_combined_advanced_and_eta_phi_energy.py" \
  --advanced-run-root "$ROOT/04_sampling/raw_outputs/active_rules_refpool1024_dense30ref_r0p01_to_1p0_step0p01_n1024_cpu60_gpu0" \
  --eta-run-root "$RUN_ROOT" \
  --graph-run-root "$STAGE/raw_outputs/eta_sweep_pilot_cpu35_gpu0" \
  --out-dir "$STAGE/figures/combined_dense30ref_rules_eta_phi_energy_r1p0_cpu60_gpu0_eta0p02_0p05_0p15_0p25" \
  --d-min 0.01 \
  --d-max 1.0 > "$LOG_DIR/plot_combined.log" 2>&1 || status=1

python "$STAGE/src/plot_eta_positive_curvature_mass_composite.py" \
  --run-root "$RUN_ROOT" \
  --out-dir "$STAGE/figures/eta_positive_curvature_mass_dense30ref_r1p0_n1024_cpu60_gpu0_eta0p02_0p05_0p15_0p25" \
  --d-min 0.01 \
  --d-max 1.0 > "$LOG_DIR/plot_eta_curvature.log" 2>&1 || status=1

date -Is > "$RUN_ROOT/ADD_ETA0P02_FINISHED_AT.txt"
exit "$status"
