...
def plot_dataset_series_grid(
    ...
    *,
    ...
    representative_rows = [sorted(rows, key=lambda row: (int(row["dataset_id"]), int(row["seed"])))[0] for _, rows in sorted(grouped.items(), key=lambda item: item[0])]
    ...
    for idx, row in enumerate(representative_rows):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_title(f"{row['cell_id']}\nbeta={float(row['beta_ising']):.2f}, p={float(row['rewire_p']):.2f}", fontsize=9)
