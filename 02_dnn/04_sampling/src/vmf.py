...
def log_sphere_mgf(dim: int, kappa: float) -> float:
    ...
    if scaled <= 0.0 or not np.isfinite(scaled):
        # Large-order Bessel values can underflow. The small-k expansion is
        # adequate for keeping smoke/candidate summaries finite; QC still sees
        # high variance through the sampled CE weights.
...
def sample_vmf_batch(mus: np.ndarray, kappa: float, rng: np.random.Generator) -> np.ndarray:
    """Draw one vMF sample around each row of ``mus`` with shared kappa."""
    ...
    # Wood's sampler separates a scalar projection from a uniform tangent
    # direction. The scalar projection distribution is independent of mu, so it
    # can be drawn once for the whole batch and then rotated row-wise.
    b = (-2.0 * float(kappa) + np.sqrt(4.0 * float(kappa) * float(kappa) + float(dim - 1) ** 2)) / float(dim - 1)
    x0 = (1.0 - b) / (1.0 + b)
    c = float(kappa) * x0 + float(dim - 1) * np.log(max(1.0 - x0 * x0, 1.0e-300))
    w = np.empty(int(n), dtype=np.float64)
    ...
    while np.any(bad):
        ...
        bad = tangent_norm[:, 0] <= 0.0
    tangent /= tangent_norm
    out = w[:, None] * mu + np.sqrt(np.maximum(1.0 - w * w, 0.0))[:, None] * tangent
