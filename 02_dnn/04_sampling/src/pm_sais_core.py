...
def sample_unit(
    *,
    ...
    if sampler in {"smc", "ce_smc", "adaptive_ce_smc", "exact_shell_l2_vmf_ce_smc"}:
        ...
        smc_payload = smc
        sampler_method = "exact_shell_l2_vmf_adaptive_ce_smc"
    else:
    ...
    if sampler_method == "exact_shell_l2_vmf_adaptive_ce_smc":
        payload.update(
            {
...
def params_from_config(config: dict[str, Any]) -> PMSAISParams:
    ...
    smc = config.get("smc") or sampling.get("smc") or {}
    ...
    method = str(config.get("method", sampling.get("method", sampling.get("sampler", "direct")))).lower()
    proposal = str(sampling.get("proposal", "")).lower()
    sampler = "adaptive_ce_smc" if "smc" in method or "smc" in proposal else "direct"
    return PMSAISParams(
