...
RUN_ROOT = ROOT / "runs" / "final" / "single_dataset_12x12_n_train_512_60ref_sparse_wide_4rule_lowtv"
...
def dense_radii() -> list[float]:
    # Sparse-wide hard-radius grid selected from the n_train_1024 dense QC profile:
    # include r0, early transitions, and broad large-radius stress points.
    return [0.01, 0.02, 0.03, 0.05, 0.08, 0.12, 0.15, 0.20, 0.30, 0.45, 0.65, 0.85, 1.00, 1.25, 1.50, 1.75, 2.00, 2.25, 2.50]


def load_config() -> dict[str, Any]:
    radii = dense_radii()
    return {
        "experiment_id": "mnist14_single_dataset_12x12_n_train_512_60ref_sparse_wide",
        "mode": "final_single_dataset",
        "identity": RUN_ROOT.name,
        "dataset": {"n_splits": 1, "n_train": 512, "n_test": 2048, "input_dim": 196, "label_rules": RULES},
        "model": {
        ...
        "sampling": {
            "r0": 0.01,
            "radii": radii,
            "radius_grid_kind": "sparse_wide_hard_from_n_train_1024_qc_profile",
            "samples_per_ref_radius": 2048,
            "lambda_reg": 1.0,
            "proposal": "exact_shell_l2_vmf_adaptive_ce_tempered_smc_sparse_wide",
            "fallback_policies_enabled": False,
            "fallback_policy_note": "Disabled for the first n_train_512 sparse-wide pass; failed rule/radius rows are diagnosed before targeted escalation.",
            "recovery_note": "Fresh n_train_512 sparse-wide run; no direct-IS recovery substitution is used in the first pass.",
        },
        "smc": {
...
def stage03_pool_design() -> None:
    ...
    plt.close(fig)
    checks = {"P": P, "expected_P": 2533, "sparse_radius_count": len(cfg["sampling"]["radii"]), "figure_exists": bool((fig_dir / "fig01_model_parameter_scale.png").exists())}
    ...
    write_text(out_dir / "REPORT.md", "# Stage 03 Pool Design\n\nFixed scaled architecture, sparse-wide hard radii, and QC gates.\n")
...
QC-failed rule/radius rows remain `no_claim`; sparse-wide raw curves are diagnostic only.
""",
    )
...
def stage06_results_figures() -> None:
    ...
    output_rows = [
        ["raw sparse-wide diagnostic", "phi_by_rule_radius_raw_diagnostic.csv", len(raw_phi_df), "no-claim where QC failed"],
    ...
    figures_rows = [
        ...
        ["raw sparse-wide energy", "figures/fig08_phi_energy_raw_dense_diagnostic.png", f"all rules, {total_radii} radii each"],
        ["raw sparse-wide full", "figures/fig09_phi_full_raw_dense_diagnostic.png", f"all rules, {total_radii} radii each"],
...
Scale: one split, {len(RULES)} label rules, {refs_per_rule} exact references per rule, sparse-wide hard d_raw grid from 0.01 to 2.50.
...
The sparse-wide raw phi(d) curves were obtained for all four rules over {total_radii} hard radii. The formal claim table should be interpreted through the QC-pass subset; raw sparse-wide figures are diagnostic wherever a rule/radius failed QC.
"""
    ...
    checks = {
        ...
        "raw_sparse_wide_figure_exists": bool((fig_dir / "fig08_phi_energy_raw_dense_diagnostic.png").exists()),
        "figure_count": len(list(fig_dir.glob("*.png"))),
    }
    ...
    write_qc("06_results_figures", "pass", checks, warnings=["Only QC-passed rule/radius rows are plotted in phi_by_rule_radius.csv; raw sparse-wide diagnostics are no-claim where QC failed."])
