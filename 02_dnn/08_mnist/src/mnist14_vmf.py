...
# Adapted into the MNIST namespace from 02_dnn/04_sampling/src/vmf.py.
def log_sphere_mgf(dim: int, kappa: float) -> float:
    """Return log E_{u uniform on S^(dim-1)} exp(kappa * mu.u)."""
    k = float(abs(kappa))
    if k == 0.0:
        return 0.0
...
__all__ = ["log_sphere_mgf", "sample_vmf", "sample_vmf_batch"]
