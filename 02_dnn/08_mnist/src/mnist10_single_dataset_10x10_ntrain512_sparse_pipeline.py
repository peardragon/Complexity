...
ARCH = MNIST14Arch(input_dim=INPUT_DIM, hidden_width=20)
P = ARCH.param_count
...
def dense_radii() -> list[float]:
    # QC-pass-oriented hard-radius line. This is intentionally narrower than
    # the previous small-line attempt because random_label reference bootstrap
    # variability exceeded the fixed QC gate by r=0.02.
    return [0.010, 0.011, 0.012, 0.013, 0.014]


def load_config() -> dict[str, Any]:
    radii = dense_radii()
    return {
        "experiment_id": "mnist10_single_dataset_10x10_box_n_train_512_60ref_microline",
        ...
        "reference_search": {"selected_refs_per_dataset": 60, "max_attempts_per_dataset": 600, "optimizer": "adam_then_lbfgs_if_needed"},
        "sampling": {
            "r0": 0.01,
            "radii": radii,
            "radius_grid_kind": "qcpass_micro_hard_line_0p010_to_0p014",
            "samples_per_ref_radius": 4096,
            "lambda_reg": 1.0,
            "proposal": "exact_shell_l2_vmf_adaptive_ce_tempered_smc_qcpass_line",
            "fallback_policies_enabled": True,
            "fallback_policy_note": "Use the existing replicated PM-SAIS fallback path for random_label selected radii, based on pilot split-logZ instability.",
            "targeted_strong_ref_ids": {
                "real_even_odd": [1, 5, 13, 22, 39, 54],
                "teacher_nn": [0, 1, 3, 13, 27],
                "random_label": [0, 2, 14],
            },
            "recovery_note": "Fresh 10x10 BOX n_train_512 run; no direct-IS recovery substitution is used in the first pass.",
        },
        "smc": {
...
def make_low_tv_spectral_teacher(
    ...
    *,
    ...
    metadata = {
        "definition": "low-frequency spectral graph teacher on MNIST10 train kNN graph with kNN interpolation for test labels",
        "graph_k": int(graph_k),
...
def box_downscale_10(raw28_batch: np.ndarray) -> np.ndarray:
    imgs = np.asarray(raw28_batch).reshape(-1, 28, 28)
    out = np.empty((imgs.shape[0], INPUT_DIM), dtype=np.float32)
    for idx, img in enumerate(imgs):
        pil_img = Image.fromarray(np.uint8(np.clip(img, 0, 255)))
        small = pil_img.resize((INPUT_SIDE, INPUT_SIDE), Image.Resampling.BOX)
        out[idx] = np.asarray(small, dtype=np.float32).reshape(-1)
    return out


def plot_dataset_figures_10x10(out_dir: Path, raw28: np.ndarray, split_rows: list[dict[str, Any]], label_df: pd.DataFrame) -> None:
    fig_dir = ensure_dir(out_dir / "figures")
    first = split_rows[0]
    train_idx = np.asarray(first["train_indices"], dtype=np.int64)
    sample_idx = train_idx[:10]
    raw10 = box_downscale_10(raw28[sample_idx]).reshape(-1, INPUT_SIDE, INPUT_SIDE)
    raw28_img = np.asarray(raw28[sample_idx]).reshape(-1, 28, 28)
    fig, axes = plt.subplots(2, len(sample_idx), figsize=(12, 2.7))
    for j in range(len(sample_idx)):
        axes[0, j].imshow(raw28_img[j], cmap="gray")
        axes[0, j].axis("off")
        axes[1, j].imshow(raw10[j], cmap="gray", interpolation="nearest")
        axes[1, j].axis("off")
    axes[0, 0].set_ylabel("28x28")
    axes[1, 0].set_ylabel("10x10")
    fig.tight_layout()
    fig.savefig(fig_dir / "fig01_mnist_28_vs_10_montage.png", dpi=160)
    plt.close(fig)
...
def plot_label_representative_figures(
    ...
    x_img = np.asarray(x_train_raw10, dtype=np.float32)
    if x_img.ndim == 2 and x_img.shape[1] == INPUT_DIM:
        x_img = x_img.reshape(-1, INPUT_SIDE, INPUT_SIDE)
    if x_img.ndim != 3 or x_img.shape[1:] != (INPUT_SIDE, INPUT_SIDE):
        raise ValueError(f"expected MNIST10 image array with shape (n, 10, 10) or (n, 100), got {x_img.shape}")
...
def stage01_dataset_prepare(*, force: bool = False) -> None:
    ...
    x_train_raw = box_downscale_10(raw28[train_idx])
    x_test_raw = box_downscale_10(raw28[test_idx])
    ...
    for rule, (y_train, y_test, metadata) in labels.items():
        ...
        pos = float(np.mean(y_train == 1))
        dataset_rows.append({"experiment_id": cfg["experiment_id"], "mode": cfg["mode"], "split_id": 0, "rule": rule, "dataset_path": rel(dataset_path), "n_train": n_train, "n_test": n_test, "input_dim": INPUT_DIM, "train_pos_fraction": pos})
    ...
    plot_dataset_figures_10x10(out_dir, raw28, [split_row], label_df)
    ...
    checks = {
        ...
        "fig01_exists": bool((out_dir / "figures" / "fig01_mnist_28_vs_10_montage.png").exists()),
    ...
    write_text(out_dir / "REPORT.md", f"# Stage 01 Dataset Prepare\n\nPrepared one MNIST10 BOX split with {len(RULES)} label rules, dataset figures, and label representative figures.\n")
...
def stage03_pool_design() -> None:
    ...
    fig_dir = ensure_dir(out_dir / "figures")
    fig, ax = plt.subplots(figsize=(6, 3.5))
    ax.bar(["old 14x14 bottleneck", "10x10 widened", "prior 2D prod"], [2533, P, 2545], color=["#9b9b9b", "#2f6f8f", "#7a9f35"])
    ax.set_ylabel("Parameter count")
    fig.tight_layout()
    fig.savefig(fig_dir / "fig01_model_parameter_scale.png", dpi=160)
    plt.close(fig)
    checks = {"P": P, "expected_P": 2461, "sparse_radius_count": len(cfg["sampling"]["radii"]), "figure_exists": bool((fig_dir / "fig01_model_parameter_scale.png").exists())}
    if checks["P"] != 2461 or not checks["figure_exists"]:
        raise StageBlocked("03_pool_design", "Pool design QC failed.", observed=checks)
    write_qc("03_pool_design", "pass", checks)
    write_text(out_dir / "REPORT.md", "# Stage 03 Pool Design\n\nFixed 10x10 widened architecture, QC-pass hard-radius line, and QC gates.\n")
...
def unit_summary_path(row: dict[str, Any], radius: float) -> Path:
    return stage_dir("05_pool2_pm_sais_sampling") / "unit_summaries" / "split_000" / str(row["rule"]) / f"ref_{int(row['ref_id']):03d}" / f"r_{float(radius):.4f}".replace(".", "p") / "unit_summary.json"
...
def sample_pm_sais_unit(row: dict[str, Any], radius: float, cfg: dict[str, Any], *, force: bool = False) -> dict[str, Any]:
    ...
    ds = load_dataset(row["dataset_path"])
    theta_ref = np.load(REPO_ROOT / str(row["theta_path"])).astype(np.float64).reshape(-1)
    seed = 3900000 + RULES.index(str(row["rule"])) * 100000 + int(row["ref_id"]) * 1000 + int(round(float(radius) * 10000))
    started = time.time()
...
def fallback_policy_for(rule: str, radius: float, ref_id: int | None = None) -> dict[str, Any] | None:
    key = (str(rule), round(float(radius), 4))
    micro_radii = {0.0100, 0.0110, 0.0120, 0.0130, 0.0140}
    rep4 = {
    ...
    rep16_4096 = {
        "name": "rep16_n4096_cess95_mh2_targeted_refs",
        "replicates": 16,
        "n_samples_each": 4096,
        "target_cess_fraction": 0.95,
        "mh_sweeps": 2,
    ...
    policy: dict[tuple[str, float], dict[str, Any]] = {
        ("real_even_odd", 0.15): rep4,
        ("real_even_odd", 1.00): rep4,
        ("teacher_nn", 0.15): rep4,
        ("teacher_nn", 0.0100): rep8_2048,
        ("teacher_nn", 0.0110): rep8_2048,
        ("teacher_nn", 0.0120): rep8_2048,
        ("teacher_nn", 0.0130): rep8_2048,
        ("teacher_nn", 0.0140): rep8_2048,
        ("teacher_nn", 2.00): rep4,
        ("random_label", 0.0100): rep16_2048,
        ("random_label", 0.0110): rep8_2048,
        ("random_label", 0.0120): rep8_2048,
        ("random_label", 0.0130): rep8_2048,
        ("random_label", 0.0140): rep8_2048,
        ("random_label", 1.00): rep4,
        ("real_even_odd", 2.00): rep8,
        ("teacher_nn", 1.00): rep8,
        ("random_label", 0.15): rep8_2048,
        ("teacher_nn", 2.50): kernel,
    }
    targeted_strong_ref_ids = {
        "real_even_odd": {1, 5, 13, 22, 39, 54},
        "teacher_nn": {0, 1, 3, 13, 27},
    ...
    if ref_id is not None and key[1] in micro_radii and int(ref_id) in targeted_strong_ref_ids.get(str(rule), set()):
        return rep16_4096
...
def sample_stage05_unit(row: dict[str, Any], radius: float, cfg: dict[str, Any], *, force: bool = False) -> dict[str, Any]:
    policy = fallback_policy_for(str(row["rule"]), float(radius), int(row["ref_id"])) if bool(cfg["sampling"].get("fallback_policies_enabled", True)) else None
    if policy is None:
    ...
    seed = 9900000 + RULES.index(str(row["rule"])) * 100000 + int(row["ref_id"]) * 1000 + int(round(float(radius) * 10000))
    started = time.time()
...
def summarize_units(unit_df: pd.DataFrame, cfg: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for (rule, radius), sub in joined.groupby(["rule", "radius"]):
        ...
        boot_sd = bootstrap_sd(sub["delta_phi_energy_unit"].to_numpy(), 49000 + RULES.index(str(rule)) * 1000 + int(round(float(radius) * 10000)))
...
def stage05_sampling_pilot(*, force: bool = False) -> None:
    ...
    radii = list(cfg["sampling"]["radii"])
    rows = []
    started = time.time()
    for row in ref_df.to_dict("records"):
        for radius in radii:
            print(f"[10x10 pilot] rule={row['rule']} ref={row['ref_id']} r={radius:.4f}", flush=True)
...
def stage05_stability_pilot(*, force: bool = False) -> None:
    ...
    for case_idx, failed in enumerate(fail_df.to_dict("records"), start=1):
        ...
        case_name = f"{str(row['rule'])}_ref{int(row['ref_id']):03d}_r{radius:.4f}".replace(".", "p")
        case_path = out_dir / "unit_summaries" / f"{case_name}.json"
        if case_path.exists() and not force:
            payload = read_json(case_path)
            payload["reused"] = True
        else:
            seed = 6100000 + RULES.index(str(row["rule"])) * 100000 + int(row["ref_id"]) * 1000 + int(round(radius * 10000))
            print(f"[stability {preset_name}] {case_idx}/{len(fail_df)} rule={row['rule']} ref={row['ref_id']} r={radius:.4f}", flush=True)
...
def stage05_stability_escalated_pilot(*, force: bool = False) -> None:
    ...
    for case_idx, failed in enumerate(fail_df.to_dict("records"), start=1):
        ...
        if case_path.exists() and not force:
            ...
            seed = 7200000 + RULES.index(str(row["rule"])) * 100000 + int(row["ref_id"]) * 1000 + int(round(radius * 10000))
            print(f"[stability {preset_name}] {case_idx}/{len(fail_df)} rule={row['rule']} ref={row['ref_id']} r={radius:.4f}", flush=True)
            payload = run_replicated_smc(row, radius, cfg, n_samples_each=n_samples_each, replicates=replicates, lambda_reg=float(cfg["sampling"]["lambda_reg"]), seed=seed)
...
def stage05_stability_particle_pilot(*, force: bool = False) -> None:
    ...
    for case_idx, failed in enumerate(fail_df.to_dict("records"), start=1):
        ...
        if case_path.exists() and not force:
            ...
            seed = 8300000 + RULES.index(str(row["rule"])) * 100000 + int(row["ref_id"]) * 1000 + int(round(radius * 10000))
            print(f"[stability {preset_name}] {case_idx}/{len(fail_df)} rule={row['rule']} ref={row['ref_id']} r={radius:.4f}", flush=True)
            payload = run_replicated_smc(row, radius, cfg, n_samples_each=n_samples_each, replicates=replicates, lambda_reg=float(cfg["sampling"]["lambda_reg"]), seed=seed)
            payload["source_preset"] = source_preset
            payload["source_split_logZ_per_P_diff"] = float(failed["split_logZ_per_P_diff"])
...
def stage05_stability_kernel_scan(*, force: bool = False) -> None:
    ...
    for failed in fail_df.to_dict("records"):
        ...
        for variant_idx, variant in enumerate(variants, start=1):
            ...
            case_name = f"{variant['name']}_{str(row['rule'])}_ref{int(row['ref_id']):03d}_r{radius:.4f}".replace(".", "p")
            case_path = out_dir / "unit_summaries" / f"{case_name}.json"
            if case_path.exists() and not force:
                payload = read_json(case_path)
                payload["reused"] = True
            else:
                seed = 9400000 + 10000 * variant_idx + RULES.index(str(row["rule"])) * 100000 + int(row["ref_id"]) * 1000 + int(round(radius * 10000))
                print(f"[kernel scan] {variant['name']} rule={row['rule']} ref={row['ref_id']} r={radius:.4f}", flush=True)
                payload = run_replicated_smc(
                    row,
                    radius,
                    cfg,
...
def stage05_pool2_pm_sais_sampling(*, force: bool = False, max_units: int | None = None, workers: int = 1) -> None:
    ...
    if worker_count == 1:
        for idx, (row, radius) in enumerate(tasks, start=1):
            if idx == 1 or idx % 100 == 0 or idx == len(tasks):
                    print(f"[10x10 stage05] unit {idx}/{len(tasks)} rule={row['rule']} ref={row['ref_id']} r={radius:.4f}", flush=True)
            rows.append(sample_stage05_unit(row, radius, cfg, force=force))
    else:
        print(f"[10x10 stage05] prepared {len(tasks)} units with workers={worker_count}", flush=True)
        with ProcessPoolExecutor(max_workers=worker_count) as executor:
            ...
            for idx, future in enumerate(as_completed(future_to_task), start=1):
                ...
                if idx == 1 or idx % 100 == 0 or idx == len(tasks):
                    ...
                    print(f"[10x10 stage05] completed {idx}/{len(tasks)} elapsed_h={elapsed / 3600.0:.2f} eta_h={remaining / 3600.0:.2f}", flush=True)
    ...
    for field, fname in [("q05_ess_fraction", "fig02_sampling_qc_ess_heatmap.png"), ("max_split_logZ_per_P_diff", "fig03_sampling_qc_split_logz_heatmap.png"), ("weighted_ce_mean", "fig05_weighted_ce_by_rule_radius.png")]:
        ...
        ticks = list(range(len(pivot.columns)))
        ax.set_xticks(ticks, [f"{pivot.columns[i]:.4f}" for i in ticks], rotation=45)
    ...
    failed_qc = qc_df[~qc_df["qc_pass"]].sort_values(["rule", "radius"]).copy()
    selected_radii = [float(r) for r in cfg["sampling"]["radii"]]
    checks = {"unit_rows": int(len(unit_df)), "expected_unit_rows": len(RULES) * int(cfg["reference_search"]["selected_refs_per_dataset"]) * len(cfg["sampling"]["radii"]), "rule_radius_rows": int(len(summary_df)), "expected_rule_radius_rows": len(RULES) * len(cfg["sampling"]["radii"]), "all_logZ_finite": bool(np.isfinite(unit_df["logZ"]).all()), "all_logZ_inf_full_finite": bool(np.isfinite(unit_df["logZ_inf_full"]).all()), "all_smc_completed": bool(unit_df["smc_completed"].all()), "hard_shell_max_abs_err": float(unit_df["hard_shell_distance_max_abs_err"].max()), "common_pass_radii_count": int(len(common_pass)), "expected_common_pass_radii_count": len(selected_radii), "failed_rule_radius_rows": int(len(failed_qc)), "fallback_policy_groups": len(policy_rows), "fallback_unit_rows": int(np.sum(unit_df.get("fallback_policy_name", "baseline") != "baseline")) if "fallback_policy_name" in unit_df else 0, "figure_count": len(list(fig_dir.glob("*.png")))}
    if checks["unit_rows"] != checks["expected_unit_rows"] or checks["rule_radius_rows"] != checks["expected_rule_radius_rows"] or not checks["all_logZ_finite"] or not checks["all_logZ_inf_full_finite"] or not checks["all_smc_completed"] or checks["hard_shell_max_abs_err"] > 1.0e-8 or checks["figure_count"] < 4:
        raise StageBlocked("05_pool2_pm_sais_sampling", "Sampling or figure QC failed.", observed=checks)
    if checks["failed_rule_radius_rows"] != 0 or checks["common_pass_radii_count"] != checks["expected_common_pass_radii_count"]:
        failed_preview = failed_qc[["rule", "radius", "q05_ess_fraction", "max_split_logZ_per_P_diff", "bootstrap_sd_phi"]].to_dict("records")
        raise StageBlocked(
            "05_pool2_pm_sais_sampling",
            "Selected 10x10 QC-pass line did not pass all Stage 05 rule/radius QC gates.",
            observed={**checks, "failed_rule_radius_rows_detail": failed_preview},
            expected={"all_selected_rule_radius_qc_pass": True, "selected_radii": selected_radii},
            next_action="Narrow the selected radius line or run a targeted stronger PM-SAIS pass for the failed rule/radius rows without modifying old retained outputs.",
        )
    sample_counts = sorted(int(v) for v in unit_df["n_samples"].dropna().unique()) if "n_samples" in unit_df else []
    sample_total_counts = {int(k): int(v) for k, v in unit_df["n_samples_total"].dropna().value_counts().sort_index().items()} if "n_samples_total" in unit_df else {}
    write_qc("05_pool2_pm_sais_sampling", "pass", checks)
    write_text(
        out_dir / "REPORT.md",
...
Fallback total-sample counts: {sample_total_counts}
...
def stage06_results_figures() -> None:
    ...
    joined["delta_phi_energy_unit"] = (joined[energy_col] - joined["logZ_r0"]) / P
    common_pass = sorted(set.intersection(*[set(qc_df[(qc_df["rule"] == rule) & (qc_df["qc_pass"])]["radius"]) for rule in RULES]))
    expected_common_pass = [float(r) for r in cfg["sampling"]["radii"]]
    failed_qc = qc_df[~qc_df["qc_pass"]].sort_values(["rule", "radius"]).copy()
    if len(failed_qc) != 0 or [float(r) for r in common_pass] != expected_common_pass:
        raise StageBlocked(
            "06_results_figures",
            "Stage 06 refused to promote figures because the selected line is not all-QC-pass.",
            observed={
                "failed_rule_radius_rows": int(len(failed_qc)),
                "failed_rule_radius_rows_detail": failed_qc[["rule", "radius", "q05_ess_fraction", "max_split_logZ_per_P_diff", "bootstrap_sd_phi"]].to_dict("records"),
                "common_pass_radii": [float(r) for r in common_pass],
            },
            expected={"all_selected_rule_radius_qc_pass": True, "selected_radii": expected_common_pass},
            next_action="Return to Stage 05 and either narrow the selected radius line or run targeted stronger PM-SAIS for failed rows.",
        )
    ...
    for (rule, radius), sub in joined.groupby(["rule", "radius"]):
        ...
        raw_sd = bootstrap_sd(values, 57000 + RULES.index(str(rule)) * 1000 + int(round(float(radius) * 10000)))
        raw_phi_rows.append(
            {
                "rule": str(rule),
                "radius": float(radius),
        ...
        sd = bootstrap_sd(values, 58000 + RULES.index(str(rule)) * 1000 + int(round(float(radius) * 10000)))
    ...
    sample_counts = {int(k): int(v) for k, v in unit_df["n_samples"].dropna().value_counts().sort_index().items()} if "n_samples" in unit_df.columns else {}
    sample_total_counts = {int(k): int(v) for k, v in unit_df["n_samples_total"].dropna().value_counts().sort_index().items()} if "n_samples_total" in unit_df.columns else {}
    ...
    for rule in RULES:
        ...
        if pass_count:
            pass_range = f"{float(passed['radius'].min()):.4f}..{float(passed['radius'].max()):.4f}"
        else:
            pass_range = "none"
        qc_rows.append([rule, pass_count, total_radii, pass_range])

    output_rows = [
        ["raw all-pass line diagnostic", "phi_by_rule_radius_raw_diagnostic.csv", len(raw_phi_df), "same rows as selected line"],
        ["QC-pass rule/radius", "phi_by_rule_radius.csv", len(phi_df), "all selected rows claimable"],
    ...
    figures_rows = [
        ...
        ["raw selected-line energy", "figures/fig08_phi_energy_raw_dense_diagnostic.png", f"all rules, {total_radii} radii each"],
        ["raw selected-line full", "figures/fig09_phi_full_raw_dense_diagnostic.png", f"all rules, {total_radii} radii each"],
    ...
    comparison_rows = [
        ["model", "2-48-48-1 tanh, P=2545", f"100-20-20-1 tanh, P={P}"],
        ...
        ["phi aggregation", "averaged over retained sample/reference units by beta/radius", f"averaged over {refs_per_rule} references for each rule/radius"],
    ...
    report = f"""# MNIST10 BOX Single-Dataset 10x10 PM-SAIS Final Report

Architecture: `100-20-20-1-tanh`, P={P}

Scale: one split, {len(RULES)} label rules, {refs_per_rule} exact references per rule, selected all-pass hard d_raw line {expected_common_pass}.
...
Sampling rows: {len(unit_df)} unit summaries. Baseline sample-count mix: {sample_counts}. Fallback total-sample mix: {sample_total_counts}.
...
{markdown_table(["item", "retained 3NN synthetic", "MNIST10 BOX run"], comparison_rows)}

## Interpretation

The selected phi(d) curves were obtained for all four rules over {total_radii} hard radii. Since every selected rule/radius row passed QC, the energy curve is the formal all-pass claim line rather than a no-claim diagnostic subset.
"""
    ...
    checks = {
        ...
        "raw_selected_line_figure_exists": bool((fig_dir / "fig08_phi_energy_raw_dense_diagnostic.png").exists()),
        "figure_count": len(list(fig_dir.glob("*.png"))),
        "all_selected_qc_pass": int(len(failed_qc)) == 0 and len(common_pass) == len(expected_common_pass),
    }
    if not checks["main_figure_exists"] or checks["figure_count"] < 3 or not checks["all_selected_qc_pass"]:
        raise StageBlocked("06_results_figures", "Final figure QC failed.", observed=checks)
    write_qc("06_results_figures", "pass", checks)
