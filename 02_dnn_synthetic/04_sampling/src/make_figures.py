...
try:
    from .make_summarized_outputs import DEFAULT_FIGURE_INPUT_ROOT, beta_from_name, beta_summary_files, clear_outputs
except ImportError:
    from make_summarized_outputs import DEFAULT_FIGURE_INPUT_ROOT, beta_from_name, beta_summary_files, clear_outputs
...
def render_figures(
    ...
    *,
    ...
    files = beta_summary_files(input_root)
    if not files:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build DNN sampling figures from figure-input CSVs.")
