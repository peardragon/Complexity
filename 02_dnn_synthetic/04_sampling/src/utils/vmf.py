...
def log_sphere_mgf(dim: int, kappa: float) -> float:
    ...
    if scaled <= 0.0 or not np.isfinite(scaled):
        # Large-order Bessel values can underflow. The small-k expansion keeps
        # production summaries finite; TPIS still exposes high-variance units
        # through the sampled CE weights.
