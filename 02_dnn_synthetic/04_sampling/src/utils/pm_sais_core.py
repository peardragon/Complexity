...
LEGACY_ADAPTIVE_CE_METHOD = "exact_shell_l2_vmf_adaptive_ce_" + "smc"
...
def _normalise_unit_payload_paths(
    ...
    *,
    ...
    if summary_path.exists() and samples_path.exists() and not force:
        ...
        if isinstance(payload, dict):
            if _normalise_unit_payload_paths(payload, record=record, unit_dir=unit_dir, samples_path=samples_path):
    ...
    if sampler in {
        ...
        smc_payload = smc
        sampler_method = TEMPERED_SMC_METHOD
    else:
    ...
    if sampler_method in {LEGACY_ADAPTIVE_CE_METHOD, TEMPERED_SMC_METHOD}:
        payload.update(
            {
