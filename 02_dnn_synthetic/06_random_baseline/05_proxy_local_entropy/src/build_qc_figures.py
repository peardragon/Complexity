...
SAMPLING_ROOT = REPO_ROOT / "02_dnn_synthetic" / "06_random_baseline" / "04_sampling" / "raw_outputs" / "shell_pool"
SAMPLING_CONFIG = REPO_ROOT / "02_dnn_synthetic" / "06_random_baseline" / "config" / "default.json"
...
def _load_records() -> list[dict[str, object]]:
    ...
    for dataset_dir in sorted(p for p in SAMPLING_ROOT.glob("dataset_*") if p.is_dir()):
        for ref_dir in sorted(p for p in dataset_dir.glob("ref_*") if p.is_dir()):
    ...
    if not records:
        raise FileNotFoundError(f"no QC unit_summary.json records were found under {SAMPLING_ROOT}")
    return records
...
def _logz_qc_rows(records: list[dict[str, object]], threshold: float) -> list[dict[str, object]]:
    ...
    for record in records:
        grouped[float(record["radius"])].append(record)

    rows: list[dict[str, object]] = []
    for radius, items in sorted(grouped.items()):
...
def _reference_variability_rows(records: list[dict[str, object]]) -> list[dict[str, object]]:
    by_dataset: dict[tuple[float, str], list[dict[str, object]]] = defaultdict(list)
    for record in records:
        by_dataset[(float(record["radius"]), str(record["dataset"]))].append(record)

    ref_sd_rows: dict[float, list[dict[str, float]]] = defaultdict(list)
    for (radius, _dataset), items in sorted(by_dataset.items()):
    ...
    rows: list[dict[str, object]] = []
    for radius, items in sorted(ref_sd_rows.items()):
        rows.append(
            {
                "beta": 0.05,
                "radius": radius,
...
def _dataset_variability_rows(records: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    dataset_means: dict[float, list[dict[str, float]]] = defaultdict(list)
    for (radius, _dataset), items in sorted(by_dataset.items()):
        logz = [float(item["logZ_inf_full"]) for item in items if np.isfinite(float(item["logZ_inf_full"]))]
        dlogz = [float(item["dlogZ_inf_full_dr"]) for item in items if np.isfinite(float(item["dlogZ_inf_full_dr"]))]
        if not logz or not dlogz:
            continue
        dataset_means[radius].append(
            {
    ...
    rows: list[dict[str, object]] = []
    for radius, items in sorted(dataset_means.items()):
...
def _plot_logz_qc(rows: list[dict[str, object]], path: Path) -> None:
    items = sorted(rows, key=lambda item: float(item["radius"]))
    radius = np.asarray([float(item["radius"]) for item in items], dtype=np.float64)
    q95 = np.asarray([float(item["q95_split_logZ_per_P_diff"]) for item in items], dtype=np.float64)
    max_split = np.asarray([float(item["max_split_logZ_per_P_diff"]) for item in items], dtype=np.float64)
    threshold = float(rows[0]["threshold_max_split_logZ_per_P_diff"])

    fig, ax = plt.subplots(figsize=(7.6, 4.7), constrained_layout=True)
    ax.plot(radius, q95, "o-", color="#2364aa", linewidth=1.4, markersize=4.2, label="q95")
    ax.plot(radius, max_split, "s-", color="#7a3e9d", linewidth=1.1, markersize=3.8, label="max")
    ax.axhline(threshold, color="#b42318", linewidth=1.1, linestyle="--", label="QC threshold")
    ax.set_xscale("log")
    ax.set_xlabel("distance d")
    ax.set_ylabel("split logZ / P diff")
    ax.set_title("Random baseline split logZ stability QC")
...
def _plot_variability(rows: list[dict[str, object]], path: Path, *, prefix: str, title: str) -> None:
    ...
    items = sorted(rows, key=lambda item: float(item["radius"]))
    radius = np.asarray([float(item["radius"]) for item in items], dtype=np.float64)
    logz = np.asarray([float(item[logz_key]) for item in items], dtype=np.float64)
    dlogz = np.asarray([float(item[dlogz_key]) for item in items], dtype=np.float64)

    fig, (ax_left, ax_right) = plt.subplots(1, 2, figsize=(10.4, 4.4), constrained_layout=True)
    ax_left.plot(radius, logz, "o-", color="#2364aa", linewidth=1.35, markersize=4.0)
    ...
    ax_right.plot(radius, dlogz, "o-", color="#7a3e9d", linewidth=1.35, markersize=4.0)
    ax_right.set_xscale("log")
    ax_right.set_xlabel("distance d")
...
def _write_report(records: list[dict[str, object]], logz_rows: list[dict[str, object]]) -> None:
    ...
    refs = sorted({str(record["ref"]) for record in records})
    method = str(run_config.get("method", ""))
    proposal = str(run_config.get("sampling", {}).get("proposal", ""))
    smc = run_config.get("smc", {})
    tempered_path_default = bool(
        run_config.get("sampling", {}).get("tempered_path_default")
        or smc.get("target_cess_fraction") is not None
        or any("smc" in value.lower() for value in methods)
    )

    report = [
        "# DNN synthetic random-baseline QC report",
        "",
        f"- method: `{method}`",
        f"- sampling proposal: `{proposal}`",
        ...
        f"- completed units: `{status.get('completed', 'unknown')}` / `{status.get('total', 'unknown')}`",
        f"- failed units: `{status.get('failed', 'unknown')}`",
        f"- QC subset records: `{len(records)}`",
        f"- QC datasets: `{len(datasets)}`",
        f"- QC refs: `{len(refs)}`",
        ...
        "",
        "- Reference variability: for each radius/dataset, compute SD of `logZ_inf_full` across references, then summarize those SDs across datasets.",
        "- Dataset variability: for each radius/dataset, average `logZ_inf_full` across references, then compute SD across datasets.",
...
def _write_cached_report(logz_rows: list[dict[str, object]]) -> None:
    ...
    report = [
        ...
        f"- method: `{run_config.get('sampling', {}).get('sampler', run_config.get('method', 'TPIS'))}`",
...
def main() -> None:
    ...
    _plot_variability(ref_rows, FIGURE_ROOT / "reference_variability_results.png", prefix="mean_ref", title="Random baseline reference variability QC")
    _plot_variability(dataset_rows, FIGURE_ROOT / "dataset_variability_results.png", prefix="dataset", title="Random baseline dataset variability QC")
